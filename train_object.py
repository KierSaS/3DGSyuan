...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if getattr(viewpoint_cam, "alpha_mask", None) is not None:
            ...
            gt_image = gt_image * alpha_mask  # 关键：GT 同样乘 alpha，避免背景牵制
        ...
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:
            ...
            # [AlphaMask-Depth] —— 深度监督也仅在前景像素上生效
            if getattr(viewpoint_cam, "alpha_mask", None) is not None:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    # [AlphaMask] —— 评估时同样应用前景掩码，保持一致性
                    if getattr(viewpoint, "alpha_mask", None) is not None:
                        am = viewpoint.alpha_mask.to("cuda")
                        image = image * am
                        gt_image = gt_image * am
